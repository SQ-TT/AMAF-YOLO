...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # ============== 卷积模块 ==============
            ...
            args = [c1, c2, *args[1:]]
        # ===================================
        elif m in [EnhancedBlock]:  # 假设这些模块需要 c1 和 c2
            c1 = ch[f]
            c2 = args[0]
            c2 = make_divisible(min(c2, max_channels) * width, 8)  # 如果需要调整宽度
            args = [c1, c2, *args[1:]]  # 正确传递 c1 和 c2

            # ============== 注意力机制 ==============
        elif m in {CBAM, GAM, LGC,  DCFA, CAB, CADP, ECA, GS_EMA, W_MSA,}:  # 自己加的注意力模块
            c1, c2 = ch[f], args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, *args[1:]]
